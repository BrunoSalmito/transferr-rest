...
import br.com.transferr.core.role.RoleTourist;
...
@Component
@Path("tourist")
public class RESTTourist extends ASuperRestClass<Tourist>{
	@Autowired
	private RoleTourist roleTourist;
	...
	public Response doGet(@PathParam("id") long id){
		Tourist entidade=null;
		try {
			entidade= roleTourist.find(id);
	...
	public Response save(Tourist exemplo){
		if(exemplo.getId()>0){
			try {
				roleTourist.update(exemplo);
			} catch (ValidationException e) {
				return RestUtil.getResponseValidationErro(e);
			}catch (Exception e) {
				return RestUtil.getResponseErroInesperado(e);
